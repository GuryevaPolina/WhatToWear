...
public class AppIntro
{
  ...
  private RectF 			m_rectBtnWeb;
    ...
    // METHODS
  AppIntro(MainActivity ctx, int language)
  {
  ...
  private void drawButton(Canvas canvas, RectF rectIn, String str, int alpha)
  {
    ...
    int colors[] = { 0, 0 };
    colors[0] = 0x92DCFE | (alpha << 24);
    colors[1] = 0x1e80B0 | (alpha << 24);
  ...
  private void drawAppleLeaf(Canvas canvas, int deltaTimeMs)
  {
    ...
    if ( (m_timeState > 2* TIME_LEAF)  )
    {
      ...
      drawButton(canvas, m_rectBtnStart, m_strStart, opa);
      if (hasWeb)
      {
        drawButton(canvas, m_rectBtnWeb, m_strWeb, opa);
      }
    }
